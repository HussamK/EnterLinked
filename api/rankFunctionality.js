...
export const categoryRank = async (uid, category) => {
    ...
    const { comparison, lessThanOrEqualList, greaterThanList } = calculateMedianAndSplitArray(categoryList);
    return {comparison, lessThanOrEqualList, greaterThanList};
}

export const updateBoundaryScores = async (review, score, uid, limitBoundary) => {
    const booklistCollectionRef = collection(FIRESTORE_DB, 'booklist', uid, review);
